...
本工具通过图像识别 + ADB 模拟点击操作游戏，**不会**也无法输入游戏内任何密码。
如果你的账号开启了「二级密码」（口袋密码 / 安全码），凡是涉及敏感操作（背包道具
使用、交易行、保护券解除等）时，工具会在密码弹窗前停下来，导致任务卡住或失败。

**解决方法**（二选一）：

1. **临时关闭**（推荐，任务跑完再开回来）：
   - 进入游戏 → 设置 → 账号与安全 → 关闭「二级密码」→ 验证手机/邮箱
2. **永久关闭**：同上

**说明**：
- 关闭二级密码不会影响账号安全（其他保护如「异地登录保护」「手机绑定」仍生效）
- 跑批完成后建议重新开启二级密码
- 本工具**不收集**也不**传输**任何账号信息，二级密码关闭期间也无密码泄露风险
...
A: 确保本项目根目录 `agent/` 下文件齐全(扁平化后 `action/` 和 `reco/` 已是子包,不是单文件):
...
四个关键文件:`custom/sink.py`、`custom/action/base.py`、`custom/reco/base.py`、`main.py`。
