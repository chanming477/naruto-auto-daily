"""tasks.advanture_task — 冒险(2026-06-30 抄自新版 MaaAutoNaruto v1.3.35 merged.json)。

narutomobile advanture.json 流程:
advanture_entry(冒险卷轴) → advanture_in_level_roll → advanture_go_fight → 自动战斗 → advanture_win → 回主页

Pipeline (8 节点):
    1. ensure_home                 Noop
    2. advanture_entry                进冒险卷轴(主页右下白底竖牌)
    3. advanture_in_level_roll                 找冒险任务卡 → 点击
    4. advanture_go_fight               出战
    5. advanture_in_fight                战斗中
    6. advanture_win                  胜利 → 点击继续
...
def _build_advanture_pipeline(nav: Navigator) -> Pipeline:
    tpls = nav.templates
    pipe = Pipeline(entry="ensure_home")

    pipe.add(Node(name="ensure_home", action=NoopAction(), next=["advanture_entry"], focus="主页基线"))

    # 1. 冒险卷轴(主页右下白底竖牌)
    pipe.add(Node(
        name="advanture_entry",
        templates=tpls("Advanture/advanture_entry.png"),
        roi=(1076, 543, 204, 177), threshold=0.85,
        action=ClickAction(x_offset=0, y_offset=0),
        next=["advanture_in_level_roll"], on_error=["back_main_screen"],
        max_hit=3, focus="点冒险卷轴",
    ))

    # 2. 找冒险任务卡
    pipe.add(Node(
        name="advanture_in_level_roll",
        templates=tpls("Advanture/level_roll.png"),
        roi=(0, 0, 363, 258), threshold=0.85,
        action=ClickAction(x_offset=0, y_offset=0),
        next=["advanture_go_fight"], on_error=["back_main_screen"],
        max_hit=3, focus="找冒险卡",
    ))

    # 3. 出战
    pipe.add(Node(
        name="advanture_go_fight",
        templates=tpls("Advanture/go_fight.png"),
        roi=(975, 419, 305, 301), threshold=0.85,
        action=ClickAction(x_offset=0, y_offset=0),
        next=["advanture_in_fight"], on_error=["back_main_screen"],
        max_hit=3, focus="出战",
    ))

    # 4. 战斗中检测
    pipe.add(Node(
        name="advanture_in_fight",
        templates=tpls("auto_battle/challenge.png"),
        roi=(560, 0, 156, 89), threshold=0.7,
        action=NoopAction(),
        next=["advanture_win"], on_error=["back_main_screen"],
    ...
    pipe.add(Node(
        ...
        roi=(341, 0, 600, 413), threshold=0.85,
        action=ClickAction(),
    ...
    pipe.add(Node(name="verify_done", action=NoopAction(), next=[], focus="冒险完成"))
    return pipe


class AdvantureTask(BaseTask):
    task_id = "advanture"
    name = "冒险"
    category = "combat"
    max_retries: int = 0
    ...
    def run(self, ctx):
        ...
        if r.success:
            return TaskResult(task_id=self.task_id, status=TaskStatus.SUCCESS, message="advanture done", attempts=1)
        self.recover(ctx); time.sleep(1)
        r2 = self._run_pipeline(adb, project_root, templates_root, log)
        if r2.success:
            return TaskResult(task_id=self.task_id, status=TaskStatus.SUCCESS, message="advanture retry", attempts=2)
        return TaskResult(task_id=self.task_id, status=TaskStatus.FAIL, message=f"advanture failed: {r2.error}", attempts=2)
    ...
    def _run_pipeline(self, adb, project_root, templates_root, log):
        ...
        return runner.run(_build_advanture_pipeline(nav), max_total_iterations=60, max_idle_iterations=10)


__all__ = ["AdvantureTask"]
