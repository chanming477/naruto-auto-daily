"""tasks.sky_ground_task — 天地(2026-06-30 抄自新版 MaaAutoNaruto v1.3.35 merged.json)。

narutomobile sky_ground.json 流程:
ninja_guide → 切天地 tab → sky_ground_entry → in_fight → 战斗 → end → back → 回主页

Pipeline (8 节点):
    1. ensure_home                 Noop
    2. ninja_guide_entry                进忍界指引卷轴
    3. sky_ground_gameplay                 找天地任务卡 → 点击
    4. sky_ground_mode               切到天地 tab
    5. sky_ground_in_fight                战斗中
    6. sky_ground_end                  胜利 → 点击继续
...
def _build_sky_ground_pipeline(nav: Navigator) -> Pipeline:
    tpls = nav.templates
    ...
    pipe.add(Node(
        ...
        next=["sky_ground_gameplay"], on_error=["back_main_screen"],
        max_hit=3, focus="点忍界指引",
    ))

    # 2. 找天地任务卡
    pipe.add(Node(
        name="sky_ground_gameplay",
        templates=tpls("Sky_ground/gameplay.png"),
        roi=(44, 382, 177, 118), threshold=0.85,
        action=ClickAction(x_offset=0, y_offset=0),
        next=["sky_ground_mode"], on_error=["back_main_screen"],
        max_hit=3, focus="找天地卡",
    ))

    # 3. 切到天地 tab
    pipe.add(Node(
        name="sky_ground_mode",
        templates=tpls("Sky_ground/sky_ground_icon.png"),
        roi=(190, 150, 913, 224), threshold=0.85,
        action=ClickAction(x_offset=0, y_offset=0),
        next=["sky_ground_in_fight"], on_error=["back_main_screen"],
        max_hit=3, focus="切到天地 tab",
    ))

    # 4. 战斗中检测
    pipe.add(Node(
        name="sky_ground_in_fight",
        templates=tpls("Weekly_win/battle_emoji.png"),
        roi=(5, 267, 79, 68), threshold=0.7,
        action=NoopAction(),
        next=["sky_ground_end"], on_error=["back_main_screen"],
    ...
    pipe.add(Node(
        name="sky_ground_end",
        templates=tpls("Sky_ground/back.png"),
        roi=(1160, 0, 119, 91), threshold=0.85,
        action=ClickAction(),
    ...
    pipe.add(Node(name="verify_done", action=NoopAction(), next=[], focus="天地完成"))
    return pipe


class SkyGroundTask(BaseTask):
    task_id = "sky_ground"
    name = "天地"
    category = "combat"
    max_retries: int = 0
    ...
    def run(self, ctx):
        ...
        if r.success:
            return TaskResult(task_id=self.task_id, status=TaskStatus.SUCCESS, message="sky_ground done", attempts=1)
        self.recover(ctx); time.sleep(1)
        r2 = self._run_pipeline(adb, project_root, templates_root, log)
        if r2.success:
            return TaskResult(task_id=self.task_id, status=TaskStatus.SUCCESS, message="sky_ground retry", attempts=2)
        return TaskResult(task_id=self.task_id, status=TaskStatus.FAIL, message=f"sky_ground failed: {r2.error}", attempts=2)
    ...
    def _run_pipeline(self, adb, project_root, templates_root, log):
        ...
        return runner.run(_build_sky_ground_pipeline(nav), max_total_iterations=60, max_idle_iterations=10)


__all__ = ["SkyGroundTask"]
