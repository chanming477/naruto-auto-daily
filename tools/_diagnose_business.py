...
from maafw_bridge import (
    MaaTaskerSingleton, get_tasker, reset_tasker, MaaEventSink,
    resolve_entry,
)
...
BIZ_HINTS = (
    'mail_', 'headhunt_', 'liveness_award_', 'activity_',
    'easy_helper_', 'rich_room_', 'ninja_book_', 'energy_',
    'use_energy_', 'advanture_', 'elite_instance_', 'team_dash_',
    'mission_office_', 'point_race_', 'weekly_win_', 'rebel_ninja_',
    'stronghold_', 'secret_realm_',
    # 忍者指引通用业务
    'ninja_guide_find_funtion_entry', 'up_swipe_for_ninja_guide',
    'down_swipe_for_ninja_guide',
)

# 弹窗/helper 节点特征
HELPER_HINTS = (
    'close_', 'check_has_x', 'check_main_screen', 'back_main_screen',
    'leave_the_team', 'level_up', 'shut_social_media', 'shut_qq',
    'im_come_back', 'im_come_back_award', 'naruto_club_x', 'text_notice',
    'group_notice', 'christmas_stocking', 'direct_hit_quit',
...
TARGETS = ['group_signin', 'liveness', 'mission_office', 'point_race',
           'weekly_win', 'stronghold']
...
def main() -> int:
    ...
    for our_tid in TARGETS:
        ...
        verdict = '✓ 真干活' if biz_nodes else ('✗ ** 假绿(只跑 helper) **' if (helper_nodes or done) else '? 未识别')
        ...
        results.append({
            'task_id': our_tid, 'entry': entry, 'status': status_label,
            'biz_count': len(biz_nodes), 'biz': sorted(biz_nodes),
            'helper_count': len(helper_nodes), 'elapsed': round(elapsed, 1),
        })
